...
description: >-
  ...
  proposes learn-then-drill, and writes progress back. Use when the user says
  面试训练, 按操盘规则继续, 知识点专场, 看专注区, 周末开练, 我有空, 继续训练,
  错题回炉, or wants to resume interview prep in this workspace.
---
...
3. **先用 2～4 句**给出：当前阶段、**当前 FZ**、本场模式（学/练）、建议动作、等价口令——**再**开练
4. **单场不跨域**：只打当前 FZ；域内按图谱**热点**（必会/高频优先）；默认先学后练
5. 学员已用口令点菜 → 按口令执行；只说人话 → 按操盘映射，并声明等价口令
...
学员说跑偏时，立刻回到：先读 CONTINUITY → 声明 FZ → 再按操盘给下一步。
