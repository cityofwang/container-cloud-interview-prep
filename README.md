# 容器云工程师面试训练

总策略：D（广覆盖）｜优先开练：A（大厂容器云 / 混部）｜方式：面次闯关 + Go 双轨  
时间盒：**平日只错题**｜**周末 3～4h 攻坚**（发 `周末开练`）｜碎片用 `我有空 30分钟`

## 进度板

| 项 | 状态 |
|----|------|
| 当前面次 | 一面（题库已就绪，待开卷） |
| 一面过关 | 未开始 |
| 二面过关 | 锁定 |
| 三面过关 | 锁定 |
| 四面过关 | 锁定 |
| Go 摸底 | 进行中（自评完；对练 03/04/06；待 07→08→09/10） |
| Go 当前小节 | （摸底定稿后：建议两者，先 basics） |
| 合面模拟次数 | 0 |
| 编码练习次数（Go 为主） | 0 / 进三面前需 ≥3 |
| 错题未巩固 | 9 |
| 能力档 | 未评估（G1/G2 暂 2） |
| 建议下周包类型 | D（先完成 Go 摸底） |
| L3 阶段 | P0 摸底收口 |
| 预计 L3 窗口 | 2026-09-22～2026-11-12 |
| 进阶解锁 | 全部锁定（E1–E5） |
| 最近岗位雷达 | 无 |
| 评估标准版本 | 2026-07-22-v1 |

## 怎么用

对话口令：`周末开练` / `今天只能看错题` / `我有空 30分钟` / `我有空 1小时` / `开始一面题库` / `开始模拟` / `复盘` / `补洞` / `编码练一题` / `过关检查` / `开始 Go 摸底` / `错题回炉` / `看错题本` / `扫岗位` / `看岗位雷达` / `能力评估` / `按雷达校准评估` / `看 L3 排期` / `看知识图谱` / `补笔记 <主题>`

- **平日**：只错题回炉（≤30 分钟）
- **周末**：`周末开练` → 按能力档组 3～4h 包（尽快冲 L3）
- 每面四步：开卷题库 → 闭卷模拟 → 复盘补洞 → 过关检查

作答写法见 `00-profile/ANSWERING.md`；能力雷达见 `00-profile/ASSESSMENT.md`；周末包说明见 `review/weekend/README.md`。  
**新对话续聊**：先读根目录 `CONTINUITY.md`（可复制其中的开场模板）。  
纠偏口令：`请严格按 CONTINUITY.md 与 docs/superpowers/specs/ 下已确认设计执行。`

## 目录

- `CONTINUITY.md` 新对话续聊一页纸（上下文满了用这个）
- `00-profile/` 背景、故事卡、作答写法、能力评估、知识图谱
- `01-round1/` … `04-round4/` 各面题库
- `05-coding-line/` 过线编码（Shell 等）
- `06-golang/` Go 双轨（摸底/小节/编码）
- `notes/` 按知识点分类的学习笔记（不会详解）
- `review/` 错题本、岗位雷达、周末包、`roadmap/`（L3 排期）
- `mocks/` 模拟记录
- `docs/superpowers/` 设计与计划

## 结构自检

```bash
bash scripts/check-structure.sh
```
