...
| 口令 | 作用 | 典型时机 |
|------|------|----------|
| `开始 Go 摸底` | 开或续 Go 摸底对练 | P0（**已完成**） |
| `开始 basics` / `解锁 basics` | 开练 `06-golang/01-basics` | **P1 当前** |
...
| `考察说明` | 口述考察十条 + 选人标准 | 随时 |
| `看 L3 排期` | 打开 P0–P3 与达标窗 / 专注区排期 | 随时 |
| `看专注区` | 当前 FZ、出口、域内热点优先序 | 随时 |
| `切换专注区 <域>` | 确认后改 CONTINUITY / L3-plan 当前 FZ | 改期/插队 |
| `知识点专场` | 先出探测题再加深讲（讲解可更长）；禁止先灌后练 | 学知识点阶段 |
...
| `补岗位 JD` | 学员发 **猎聘/Boss** JD 截图或文字 → 解读落库 + patch 技能清单/图谱 | 投递前/猎聘上看到心仪岗 |
...
| 日期 | 变更 |
|------|------|
| 2026-07-27 | 操盘铁律：**先答后讲**（禁止开场灌输） |
| 2026-07-27 | 专注区 C + 热点；口令 `看专注区` / `切换专注区` / `知识点专场` |
| 2026-07-26 | 教练 Skill + 续聊脚本；口令摘要增补 `面试训练` |
